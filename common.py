...
def progress_bar(progress=0, status="", bar_len=20):
    ...
    text = "\rProgress: [{}] {:.2f}% {}".format(
        COLORS['darkgray_bg'] + " "*(block) + COLORS['end'] + COLORS['black'] + ' '*(bar_len-block) + COLORS['end'], 
...
def init_experiment(args, seed=420):
    ...
    output_dir = os.path.join("outputs", args["agent"], args["output"])
...
class ScreenRecorder:
    ''' For recording videos of your amazing work! '''

    def __init__(self, output_file, fps=20.0):
        SCREEN_SIZE = tuple(pyautogui.size())
        fourcc = cv2.VideoWriter_fourcc(*"XVID")
        self.out = cv2.VideoWriter(output_file, fourcc, fps, (SCREEN_SIZE))
